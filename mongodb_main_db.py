...
def run_db():
    ...
    for message in consumer:
        ...
        # Insert into DB_NOSPAM
        db = create_db(DEFAULT_URI, DB_NAME)
        col_nospam = db[COL_NOSPAM]
        #col_nospam.insert_one(record)
        col_nospam.insert_one(message.value)
        log.info(f"Inserted record into {COL_NOSPAM}: {message.value}")

        for x in col_nospam.find():
            log.info(x)
